...
function HeaderGPI({ isMobile = false }: HeaderGPIProps) {
  ...
  return (
    <>
      {isMobile ? (
 <nav>
          {/* Header con el color verde de PulgaShop */}
          <div className="h-16 w-full" style={{ backgroundColor: '#00D563' }}>
            <div className="flex w-full h-full items-center justify-between px-6 py-0">
              
              {/* Contenedor del ícono de hamburguesa - IZQUIERDA */}
              <div
                onClick={openMenuAction}
                className="cursor-pointer p-2 text-white hover:bg-white hover:bg-opacity-20 rounded-lg transition-all"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
              ...
              {/* Logo en el centro */}
              <div className="absolute left-1/2 transform -translate-x-1/2 cursor-pointer" onClick={() => redirectTo('/home')}>
                <img src={logo} alt="PulgaShop logo" className="h-10 w-auto" />
              </div>
